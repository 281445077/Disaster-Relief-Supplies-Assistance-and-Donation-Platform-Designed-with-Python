...
def getWhere(request,qs):
    ...
    if request.GET.get("leixing"):
        qs = qs.filter(leixing=request.GET.get("leixing"))
    if request.GET.get("beizhuxinxi"):
        qs = qs.filter(beizhuxinxi__contains=request.GET.get("beizhuxinxi"))
...
def adminlist(request):
    qs = getWhere(request,Chuli.objects)
    ...
    return render(request , "chuli/admin/list.html" , locals() , )
# 捐赠人列表页面
def juanzengren(request):
    ...
    return render(request , "chuli/admin/juanzengren.html" , locals() , )
...
def adminadd(request):
    ...
    return render(request , "chuli/admin/add.html",locals())
...
def adminupdt(request):
    id = request.GET.get("id")
    mmm = Chuli.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "chuli/admin/updt.html" , locals())
...
def admindetail(request):
    id = request.GET.get("id")
    map = Chuli.objects.get(pk=id)
    return render(request , "chuli/admin/detail.html" , locals())
...
def insert(request):
    ...
    data = {
        ...
        'leixing': utils.input(request,'leixing',''),
        'beizhuxinxi': utils.input(request,'beizhuxinxi',''),
    ...
    try:
        commonModels.execute("update  juanzeng set zhuangtai='已处理' where  bianhao='%s'" % (request.POST.get("bianhao")))
...
def update(request):
    ...
    post = request.POST.copy()
    old = Chuli.objects.get(pk = charuid)
    data = {
        'id': charuid,
        'juanjiid_id': utils.input(request,'juanjiid',utils.input(request,'juanjiid_id',0)),
        'juanzengid_id': utils.input(request,'juanzengid',utils.input(request,'juanzengid_id',0)),
        ...
        'leixing': utils.input(request,'leixing',old.leixing),
        'beizhuxinxi': utils.input(request,'beizhuxinxi',old.beizhuxinxi),
