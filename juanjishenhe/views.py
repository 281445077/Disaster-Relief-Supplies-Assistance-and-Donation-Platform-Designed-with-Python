...
def adminlist(request):
    qs = getWhere(request,Juanjishenhe.objects)
    ...
    return render(request , "juanjishenhe/admin/list.html" , locals() , )
# 捐赠人列表页面
def juanzengren(request):
    ...
    return render(request , "juanjishenhe/admin/juanzengren.html" , locals() , )
...
def adminadd(request):
    ...
    if 'id' in request.GET:
        ...
        readMap = Juanji.objects.get(pk = id)
    return render(request , "juanjishenhe/admin/add.html",locals())
...
def adminupdt(request):
    id = request.GET.get("id")
    mmm = Juanjishenhe.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "juanjishenhe/admin/updt.html" , locals())
...
def admindetail(request):
    id = request.GET.get("id")
    map = Juanjishenhe.objects.get(pk=id)
    return render(request , "juanjishenhe/admin/detail.html" , locals())
...
def insert(request):
    ...
    data = {
        'juanjiid_id': utils.input(request,'juanjiid',utils.input(request,'juanjiid_id',0)),
    ...
    model = Juanjishenhe(**data)
    model.save(force_insert = True)

    try:
        commonModels.execute("update juanji set juanjizhuangtai='%s' where  id='%s'" % (request.POST.get("shenhejieguo"),request.POST.get("juanjiid")))
        commonModels.execute("update  juanzeng set zhuangtai='未被领取' where bianhao='%s' and '%s'='已通过'" % (request.POST.get("bianhao"),request.POST.get("shenhejieguo")))
        commonModels.execute("update  juanzeng set zhuangtai='未通过' where  bianhao='%s' and '%s'='未通过'" % (request.POST.get("bianhao"),request.POST.get("shenhejieguo")))
...
def update(request):
    ...
    post = request.POST.copy()
    old = Juanjishenhe.objects.get(pk = charuid)
    data = {
        'id': charuid,
        'juanjiid_id': utils.input(request,'juanjiid',utils.input(request,'juanjiid_id',0)),
        'juanzengid_id': utils.input(request,'juanzengid',utils.input(request,'juanzengid_id',0)),
