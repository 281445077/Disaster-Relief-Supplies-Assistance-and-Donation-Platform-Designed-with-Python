...
from .models import Juanzengshenhe
...
def adminlist(request):
    qs = getWhere(request,Juanzengshenhe.objects)
    ...
    return render(request , "juanzengshenhe/admin/list.html" , locals() , )
# 捐赠人列表页面
def juanzengren(request):
    ...
    return render(request , "juanzengshenhe/admin/juanzengren.html" , locals() , )
...
def adminadd(request):
    ...
    return render(request , "juanzengshenhe/admin/add.html",locals())
...
def adminupdt(request):
    id = request.GET.get("id")
    mmm = Juanzengshenhe.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "juanzengshenhe/admin/updt.html" , locals())
...
def admindetail(request):
    id = request.GET.get("id")
    map = Juanzengshenhe.objects.get(pk=id)
    return render(request , "juanzengshenhe/admin/detail.html" , locals())
...
def insert(request):
    ...
    model = Juanzengshenhe(**data)
    model.save(force_insert = True)

    try:
        commonModels.execute("update  juanzeng set zhuangtai='%s' where  id ='%s'" % (request.POST.get("shenhejieguo"),request.POST.get("juanzengid")))
        commonModels.execute("update  yonghu set aixinjifen=aixinjifen+1 where  yonghuming='%s'" % (request.POST.get("juanzengren")))
...
def update(request):
    ...
    post = request.POST.copy()
    old = Juanzengshenhe.objects.get(pk = charuid)
    data = {
        'id': charuid,
        'juanzengid_id': utils.input(request,'juanzengid',utils.input(request,'juanzengid_id',0)),
