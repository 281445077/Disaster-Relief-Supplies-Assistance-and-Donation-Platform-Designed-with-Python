...
def adminlist(request):
    qs = getWhere(request,Lunbotu.objects)
    ...
    return render(request , "lunbotu/admin/list.html" , locals() , )
...
def adminadd(request):
    ...
    return render(request , "lunbotu/admin/add.html",locals())
...
def adminupdt(request):
    ...
    mmm = Lunbotu.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "lunbotu/admin/updt.html" , locals())
...
def update(request):
    ...
    post = request.POST.copy()
    old = Lunbotu.objects.get(pk = charuid)
    data = {
        'id': charuid,
