...
def getWhere(request,qs):
    ...
    if request.GET.get("qianshoubeizhu"):
        qs = qs.filter(qianshoubeizhu__contains=request.GET.get("qianshoubeizhu"))
...
def adminlist(request):
    qs = getWhere(request,Qianshou.objects)
    ...
    return render(request , "qianshou/admin/list.html" , locals() , )
# 捐赠人列表页面
def juanzengren(request):
    ...
    return render(request , "qianshou/admin/juanzengren.html" , locals() , )
# 申请用户列表页面
def shenqingyonghu(request):
    ...
    return render(request , "qianshou/admin/shenqingyonghu.html" , locals() , )
...
def adminadd(request):
    ...
    if 'id' in request.GET:
        ...
        readMap = Paiji.objects.get(pk = id)
    return render(request , "qianshou/admin/add.html",locals())
...
def adminupdt(request):
    id = request.GET.get("id")
    mmm = Qianshou.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "qianshou/admin/updt.html" , locals())
...
def admindetail(request):
    id = request.GET.get("id")
    map = Qianshou.objects.get(pk=id)
    return render(request , "qianshou/admin/detail.html" , locals())
...
def insert(request):
    ...
    data = {
        'paijiid_id': utils.input(request,'paijiid',utils.input(request,'paijiid_id',0)),
        ...
        'qianshoubeizhu': utils.input(request,'qianshoubeizhu',''),
...
def update(request):
    ...
    post = request.POST.copy()
    old = Qianshou.objects.get(pk = charuid)
    data = {
        'id': charuid,
        'paijiid_id': utils.input(request,'paijiid',utils.input(request,'paijiid_id',0)),
        'shenlingid_id': utils.input(request,'shenlingid',utils.input(request,'shenlingid_id',0)),
        ...
        'qianshoubeizhu': utils.input(request,'qianshoubeizhu',old.qianshoubeizhu),
        'addtime': old.addtime,
