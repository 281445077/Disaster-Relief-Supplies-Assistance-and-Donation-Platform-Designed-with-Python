...
from .models import Wupinleibie
...
def getWhere(request,qs):
    if request.GET.get("leibieming"):
        qs = qs.filter(leibieming__contains=request.GET.get("leibieming"))
...
def adminlist(request):
    qs = getWhere(request,Wupinleibie.objects)
    ...
    return render(request , "wupinleibie/admin/list.html" , locals() , )
...
def adminadd(request):
    ...
    return render(request , "wupinleibie/admin/add.html",locals())
...
def adminupdt(request):
    ...
    mmm = Wupinleibie.objects.get(pk = id)
    if mmm == None:
        return utils.showError(request,"没有找到相关数据")


    return render(request, "wupinleibie/admin/updt.html" , locals())
...
def delete(request):
    ...
    for id in ids:
        map = Wupinleibie.objects.get(pk = id)
...
def insert(request):
    ...
    data = {
        'leibieming': utils.input(request,'leibieming',''),

    }


    model = Wupinleibie(**data)
...
def update(request):
    ...
    post = request.POST.copy()
    old = Wupinleibie.objects.get(pk = charuid)
    data = {
        'id': charuid,
        'leibieming': utils.input(request,'leibieming',old.leibieming),
